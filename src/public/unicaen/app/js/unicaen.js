...
/* ================================================== Collapsable Actions ==================================================== */
/**
 * @desc objectif : permettre de regrouper un ensemble d'actions
 * @constructor
 */
$.widget("unicaen.collaspableActions", {
    options: {
        toggler : '<a><span class="fas fa-ellipsis"></span></a>',
        //Pour rajouter des class aux différents elements
        togglerClass : 'btn',
        ulClass : '',
        liClass : '',
        autoToggle: false,
    }, //pour de futur amélioration
    ...
    _create: function ()
    {
        ...
        this.initOptions();
        this.__make();
        this.element.attr('data-bs-display', true);

        if (this.options.autoToggle) {
            setTimeout(function() { that.toggle(); }, 100);
        }
    },
    ...
    __make: function () {
        ...
        var toggler = $(this.options.toggler);
        toggler.addClass('dropdown-toggle');
        var ul = $('<ul class="dropdown-menu"></ul>');

        if (typeof toggler.data('bs-toggle') === 'undefined') {
        ...
        this.element.append(toggler);
        this.element.append(ul);

        //Rajout des classes optionnelles
        if(this.options.togglerClass !== 'undefined'){
            toggler.addClass(this.options.togglerClass);
        }
        if(this.options.ulClass !== 'undefined'){
            ul.addClass(this.options.ulClass);
        }
        if(this.options.liClass !== 'undefined'){
            ul.find('> li').addClass(this.options.liClass);
        }
    },
...
$(function ()
{
    WidgetInitializer.add('collapsable-actions', 'collaspableActions');
});
